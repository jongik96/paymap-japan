import { MapPin, CreditCard, Star, MessageCircle } from "lucide-react";

export default function Home() {
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
      {/* Header */}
      <header className="bg-white shadow-sm border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-4">
            <div className="flex items-center space-x-2">
              <MapPin className="h-8 w-8 text-blue-600" />
              <h1 className="text-2xl font-bold text-gray-900">PayMap Japan</h1>
            </div>
            <p className="text-sm text-gray-600">Payment Method Review Service</p>
          </div>
        </div>
      </header>

      {/* Hero Section */}
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="text-center mb-12">
          <h2 className="text-4xl font-bold text-gray-900 mb-4">
            Discover Payment Methods at Japanese Restaurants
          </h2>
          <p className="text-xl text-gray-600 mb-8">
            Find restaurants on Google Maps, check available payment methods, 
            and share your experiences with others
          </p>
          <div className="flex justify-center space-x-4">
            <a href="/map" className="bg-blue-600 text-white px-6 py-3 rounded-lg hover:bg-blue-700 transition-colors flex items-center space-x-2">
              <MapPin className="h-5 w-5" />
              <span>View Map</span>
            </a>
            <a href="/map" className="bg-gray-100 text-gray-700 px-6 py-3 rounded-lg hover:bg-gray-200 transition-colors flex items-center space-x-2">
              <MessageCircle className="h-5 w-5" />
              <span>Write Review</span>
            </a>
          </div>
          
          <div className="flex justify-center mt-8">
            <a href="/admin" className="text-gray-500 hover:text-gray-700 text-sm underline">
              관리자 대시보드 →
            </a>
          </div>
        </div>

        {/* Features */}
        <div className="grid md:grid-cols-3 gap-8 mb-12">
          <div className="bg-white p-6 rounded-lg shadow-sm border">
            <div className="flex items-center justify-center w-12 h-12 bg-blue-100 rounded-lg mb-4">
              <MapPin className="h-6 w-6 text-blue-600" />
            </div>
            <h3 className="text-lg font-semibold text-gray-900 mb-2">Map-Based Search</h3>
            <p className="text-gray-600">Easily find restaurants in your desired area and check their locations on Google Maps</p>
          </div>
          
          <div className="bg-white p-6 rounded-lg shadow-sm border">
            <div className="flex items-center justify-center w-12 h-12 bg-green-100 rounded-lg mb-4">
              <CreditCard className="h-6 w-6 text-green-600" />
            </div>
            <h3 className="text-lg font-semibold text-gray-900 mb-2">Payment Method Info</h3>
            <p className="text-gray-600">Check available payment methods at each restaurant at a glance</p>
          </div>
          
          <div className="bg-white p-6 rounded-lg shadow-sm border">
            <div className="flex items-center justify-center w-12 h-12 bg-purple-100 rounded-lg mb-4">
              <Star className="h-6 w-6 text-purple-600" />
            </div>
            <h3 className="text-lg font-semibold text-gray-900 mb-2">Real-time Reviews</h3>
            <p className="text-gray-600">Leave anonymous reviews about payment methods and share them with others</p>
          </div>
        </div>

        {/* How it works */}
        <div className="bg-white rounded-lg shadow-sm border p-8">
          <h3 className="text-2xl font-bold text-gray-900 mb-6 text-center">How to Use</h3>
          <div className="grid md:grid-cols-4 gap-6">
            <div className="text-center">
              <div className="w-12 h-12 bg-blue-600 text-white rounded-full flex items-center justify-center mx-auto mb-3 text-lg font-bold">1</div>
              <h4 className="font-semibold text-gray-900 mb-2">Find Restaurants</h4>
              <p className="text-sm text-gray-600">Browse the map to find restaurants in your desired area</p>
            </div>
            <div className="text-center">
              <div className="w-12 h-12 bg-blue-600 text-white rounded-full flex items-center justify-center mx-auto mb-3 text-lg font-bold">2</div>
              <h4 className="font-semibold text-gray-900 mb-2">Click Marker</h4>
              <p className="text-sm text-gray-600">Click on restaurant markers to view detailed information</p>
            </div>
            <div className="text-center">
              <div className="w-12 h-12 bg-blue-600 text-white rounded-full flex items-center justify-center mx-auto mb-3 text-lg font-bold">3</div>
              <h4 className="font-semibold text-gray-900 mb-2">Check Payment</h4>
              <p className="text-sm text-gray-600">View available payment methods and other people&apos;s reviews</p>
            </div>
            <div className="text-center">
              <div className="w-12 h-12 bg-blue-600 text-white rounded-full flex items-center justify-center mx-auto mb-3 text-lg font-bold">4</div>
              <h4 className="font-semibold text-gray-900 mb-2">Write Review</h4>
              <p className="text-sm text-gray-600">Share your experience by writing a review</p>
            </div>
          </div>
        </div>
      </main>

      {/* Footer */}
      <footer className="bg-gray-50 border-t mt-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="text-center text-gray-600">
            <p>&copy; 2025 PayMap Japan. Community for Payment Method in Japan</p>
          </div>
        </div>
      </footer>
    </div>
  );
}
